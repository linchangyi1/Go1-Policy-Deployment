...
import copy
import time
import mocap.data_descriptions as data_descriptions
import mocap.mocap_data as mocap_data
...
class NatNetClient:
    ...
    def __unpack_rigid_body( self, data, major, minor, rb_num, frame_number):
        offset = 0
        ...
        rigid_body = mocap_data.RigidBody(new_id, pos, rot)

        # Send information to any listener.
        if self.rigid_body_listener is not None:
            self.rigid_body_listener( new_id, pos, rot, frame_number%100 )
        ...
        if( major < 3  and major != 0) :
            ...
            for i in marker_count_range:
                rb_marker_list.append(mocap_data.RigidBodyMarker())
    ...
    def __unpack_skeleton( self, data, major, minor, skeleton_num=0):
        ...
        skeleton = mocap_data.Skeleton(new_id)
...
#Unpack Mocap Data Functions
    def __unpack_frame_prefix_data( self, data):
        ...
        frame_prefix_data=mocap_data.FramePrefixData(frame_number)
    ...
    def __unpack_legacy_other_markers( self, data, packet_size, major, minor):
        ...
        other_marker_data = mocap_data.LegacyMarkerData()
    ...
    def __unpack_marker_set_data( self, data, packet_size, major, minor):
        marker_set_data=mocap_data.MarkerSetData()
        offset = 0
        # Markerset count (4 bytes)
        ...
        for i in range( 0, marker_set_count ):
            marker_data = mocap_data.MarkerData()
            # Model name
    ...
    def __unpack_rigid_body_data( self, data, packet_size, major, minor, frame_number):
        rigid_body_data = mocap_data.RigidBodyData()
        offset = 0
        ...
        for i in range( 0, rigid_body_count ):
            offset_tmp, rigid_body = self.__unpack_rigid_body( data[offset:], major, minor, i, frame_number)
            offset += offset_tmp
    ...
    def __unpack_skeleton_data( self, data, packet_size, major, minor):
        skeleton_data = mocap_data.SkeletonData()
    ...
    def __unpack_labeled_marker_data( self, data, packet_size, major, minor):
        labeled_marker_data = mocap_data.LabeledMarkerData()
        offset = 0
        ...
        if( ( major == 2 and minor > 3 ) or major > 2 ):
            ...
            for lm_num in range( 0, labeled_marker_count ):
                ...
                labeled_marker = mocap_data.LabeledMarker(tmp_id,pos,size,param, residual)
    ...
    def __unpack_force_plate_data( self, data, packet_size, major, minor):
        force_plate_data = mocap_data.ForcePlateData()
        n_frames_show_max = 4
        offset = 0
        ...
        if( ( major == 2 and minor >= 9 ) or major > 2 ):
            ...
            for i in range( 0, force_plate_count ):
                ...
                offset += 4
                force_plate = mocap_data.ForcePlate(force_plate_id)
                ...
                for j in range( force_plate_channel_count ):
                    fp_channel_data = mocap_data.ForcePlateChannelData()
    ...
    def __unpack_device_data( self, data, packet_size, major, minor):
        device_data = mocap_data.DeviceData()
        n_frames_show_max = 4
        offset = 0
        ...
        if ( major == 2 and minor >= 11 ) or (major > 2) :
            ...
            for i in range( 0, device_count ):
                ...
                offset += 4
                device = mocap_data.Device(device_id)
                # Channel Count
                ...
                for j in range( 0, device_channel_count ):
                    device_channel_data = mocap_data.DeviceChannelData()
    ...
    def __unpack_frame_suffix_data( self, data, packet_size, major, minor):
        frame_suffix_data = mocap_data.FrameSuffixData()
        offset = 0
    ...
    def __unpack_mocap_data( self, data : bytes, packet_size, major, minor):
        mocap_data_instance = mocap_data.MoCapData()
        ...
        offset += rel_offset
        mocap_data_instance.set_prefix_data(frame_prefix_data)
        ...
        offset += rel_offset
        mocap_data_instance.set_marker_set_data(marker_set_data)
        ...
        offset += rel_offset
        mocap_data_instance.set_legacy_other_markers(legacy_other_markers)
        ...
        # Rigid Body Data
        rel_offset, rigid_body_data = self.__unpack_rigid_body_data(data[offset:], (packet_size - offset),major, minor, frame_number)
        offset += rel_offset
        mocap_data_instance.set_rigid_body_data(rigid_body_data)
        ...
        offset += rel_offset
        mocap_data_instance.set_skeleton_data(skeleton_data)
        ...
        if (((major == 4) and (minor > 0)) or (major > 4)):
            ...
            offset += rel_offset
            mocap_data_instance.set_asset_data(asset_data)
        ...
        offset += rel_offset
        mocap_data_instance.set_labeled_marker_data(labeled_marker_data)
        ...
        offset += rel_offset
        mocap_data_instance.set_force_plate_data(force_plate_data)

        # Device Data
        rel_offset,device_data = self.__unpack_device_data(data[offset:], (packet_size - offset),major, minor)
        offset += rel_offset
        mocap_data_instance.set_device_data(device_data)
        ...
        offset += rel_offset
        mocap_data_instance.set_suffix_data(frame_suffix_data)
        ...
        return offset, mocap_data_instance
    ...
    def __unpack_marker_description( self, data, major, minor ):
        ...
        marker_desc=data_descriptions.MarkerDescription(name, marker_id, initialPosition, marker_size, marker_params)
    ...
    def __unpack_asset_rigid_body_data( self, data, major, minor ):
        ...
        # Package for return object
        rigid_body_data=mocap_data.AssetRigidBodyData(rbID, pos, rot, mean_error, marker_params)
    ...
    def __unpack_asset_marker_data( self, data, major, minor ):
        ...
        marker_data = mocap_data.AssetMarkerData(marker_id, pos, marker_size, marker_params, residual)
        return offset, marker_data

    def __unpack_asset_data( self, data, packet_size, major, minor):
        asset_data = mocap_data.AssetData()
    ...
    def __unpack_asset_description(self, data, major, minor):
        ...
        # package for output
        asset_desc = data_descriptions.AssetDescription(name, assetType, assetID, rigidbodyArray, markerArray)
